...
const runtimeConfig: RuntimeConfig = {
	...
	transportMode:
		...
		parseTransportMode(
			process.env.PRESTASHOP_MCP_TRANSPORT ?? process.env.MCP_TRANSPORT,
			"stdio",
		),
	httpHost: cliArgs.httpHost ?? process.env.HOST ?? DEFAULT_HTTP_HOST,
	httpPort:
		cliArgs.httpPort ??
		parsePositiveInteger(process.env.PORT, DEFAULT_HTTP_PORT),
	httpPath: normalizeHttpPath(
		cliArgs.httpPath ?? process.env.MCP_HTTP_PATH ?? DEFAULT_HTTP_PATH,
	),
};
...
const ResourceNameSchema = z
	...
	.regex(
		/^[A-Za-z0-9_]+$/,
		"Use a PrestaShop Webservice resource name such as products, customers, orders, or cart_rules.",
	)
	.describe(
		"PrestaShop Webservice resource name, for example products, customers, orders, or cart_rules.",
	);
...
const ResourceIdSchema = z
	...
	.regex(
		/^[A-Za-z0-9_-]+$/,
		"Use the resource ID exactly as exposed by PrestaShop.",
	)
		...
		"prestashop_get_resource_schema",
		{
			...
			inputSchema: {
				...
				schema: z
					...
					.default("synopsis")
					.describe(
						"Use synopsis for field metadata or blank for an XML payload template.",
					),
				response_format: ResponseFormatSchema,
			},
		...
		async ({ resource, schema, response_format }) => {
			try {
				const response = await requestPrestaShop({
					...
					query: {
						schema,
						...(response_format === "json" ? { output_format: "JSON" } : {}),
					},
					responseFormat: response_format,
				});
		...
		"prestashop_list_resources",
		{
			...
			inputSchema: {
				...
				display: z
					...
					.optional()
					.describe(
						"PrestaShop display selector, for example full, [id,name], or [id,date_add,total_paid]. Omit to use the store default.",
					),
				filters: z
					.record(z.string().min(1), z.string())
					.optional()
					.describe(
						'PrestaShop filters. The key becomes filter[key], for example {"id": "[1|2]", "date_add": "[2024-01-01,2024-12-31]"}.',
					),
				sort: z
					.string()
					.min(1)
					.optional()
					.describe(
						"PrestaShop sort expression, for example [id_ASC] or [date_add_DESC].",
					),
				limit: z
					.string()
					.min(1)
		...
		"prestashop_create_resource",
		{
			...
			inputSchema: {
				...
				xml: z
					...
					.min(1)
					.describe(
						"Complete PrestaShop XML payload, usually rooted at <prestashop><resource>...</resource></prestashop>.",
					),
				extra_query: QueryRecordSchema.optional(),
				response_format: ResponseFormatSchema,
			},
		...
		"prestashop_update_resource",
		{
			...
			inputSchema: {
				...
				xml: z
					...
					.min(1)
					.describe(
						"Complete PrestaShop XML payload for the updated entity, usually including the id field.",
					),
				extra_query: QueryRecordSchema.optional(),
				response_format: ResponseFormatSchema,
			},
		...
		"prestashop_request",
		{
			...
			inputSchema: {
				...
				path: z
					...
					.min(1)
					.describe(
						"Path below /api, for example products, products/1, or images/products/1. Do not include the store domain.",
					),
...
async function startStdioServer(): Promise<void> {
	...
	console.error(
		`${SERVER_NAME} running on stdio for ${safeStoreLabel(runtimeConfig.baseUrl)}.`,
	);
}
...
async function startHttpServer(): Promise<void> {
	...
	const displayHost =
		runtimeConfig.httpHost === "0.0.0.0" ? "localhost" : runtimeConfig.httpHost;
	console.error(
	...
	if (request.method !== "POST") {
		...
		sendJson(response, 405, {
			...
			error: {
				code: -32000,
				message:
					"Method not allowed. Use POST for stateless MCP Streamable HTTP requests.",
			},
			id: null,
		});
		return;
...
function parseCliArgs(args: string[]): ParsedCliArgs {
	...
	for (let index = 0; index < args.length; index += 1) {
		...
		if (arg.startsWith("--transport=")) {
			parsed.transportMode = parseTransportMode(
		...
		if (arg.startsWith("--port=")) {
			parsed.httpPort = parsePositiveInteger(
...
function printHelp(): void {
	console.log(
		[
			"PrestaShop Webservice MCP relay",
			"",
			"Usage:",
			"  prestashop-webservice-relay-mcp --base-url https://yourstore.com --key YOUR_PRESTASHOP_WEBSERVICE_KEY",
			"  prestashop-webservice-relay-mcp --transport http --port 3000 --base-url https://yourstore.com --key YOUR_PRESTASHOP_WEBSERVICE_KEY",
			"",
...
function sanitizeApiPath(path: string): string {
	...
	if (withoutApiPrefix.includes("?") || withoutApiPrefix.includes("#")) {
		throw new Error(
			"Pass query parameters through the query or extra_query input instead of embedding them in path.",
		);
	}

	if (
		/^[a-z][a-z0-9+.-]*:/i.test(withoutApiPrefix) ||
		withoutApiPrefix.split("/").includes("..")
	) {
		throw new Error(
			"Path must be relative to the configured PrestaShop /api endpoint.",
		);
	}
...
function withResponseFormat(
	query: Record<string, string> | QueryParams,
	responseFormat: ResponseFormat,
): QueryParams {
...
function parseResponseBody(
	...
	contentType: string,
	responseFormat: ResponseFormat,
): { format: string; data?: unknown; rawBody?: string } {
	if (!body.trim()) {
